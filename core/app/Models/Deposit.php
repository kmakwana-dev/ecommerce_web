...
class Deposit extends Model
{
    protected $casts = [
        'detail' => 'object',
        'success_route_params' => 'array',
        'failed_route_params' => 'array',
    ];
    ...
    {
        return $this->belongsTo(User::class)->withDefault(function ($user, $deposit) {
            return $deposit->order->user;
        });
    }
    ...
    public function customer() {
        return $this->belongsTo(User::class)->withDefault(function ($user, $deposit) {
            ...
            $user = new User();
            $user->email = $order->guest?->email;
            $user->fisrtname = strstr($user->email, '@', true);
            $user->lastname = '';
            $user->country_name = $order->guest?->country_name;
            $user->dial_code = $order->guest?->dial_code;
            $user->country_code = $order->guest?->country_code;
            return $user;
        });
    }
    ...
    public function methodName(){
        if ($this->method_code < 5000) {
            $methodName = isset($this->gatewayCurrency()->name) ? $this->gatewayCurrency()->name : '';
        }else{
