    ...
    public function storeBulkImport(Request $request) {
        ...
        try {
            ...
            foreach ($rows->allData as $data) {
                ...
                if (!empty($combinedData['main_image_id']) && filter_var($combinedData['main_image_id'], FILTER_VALIDATE_URL)) {
                    $image_url = $combinedData['main_image_id'];
                    $ext = pathinfo(parse_url($image_url, PHP_URL_PATH), PATHINFO_EXTENSION) ?: 'jpg';
                    $filename = uniqid() . '.' . $ext;
                    $image_dir = base_path('../assets/images/product');
                    if (!file_exists($image_dir)) mkdir($image_dir, 0755, true);
                    
                    $img_data = @file_get_contents($image_url);
                    if ($img_data) {
                        file_put_contents($image_dir . '/' . $filename, $img_data);
                        copy($image_dir . '/' . $filename, $image_dir . '/thumb_' . $filename);
                        ...
                        $combinedData['main_image_id'] = $media_id;
                        
                        // Add to gallery images data so it gets linked in media_product
                        $galleryImagesData[] = [
                            'slug'      => $combinedData['slug'],
                            'media_ids' => [$media_id]
                        ];
                    } else {
                        $combinedData['main_image_id'] = null;
                    }
                }

                if (!empty($combinedData['gallery_images'])) {
                    $galleryImages = explode(',', $combinedData['gallery_images']);
                    $newGalleryIds = [];
                    foreach ($galleryImages as $img) {
                        ...
                        if (filter_var($img, FILTER_VALIDATE_URL)) {
                            ...
                            // It's a regular ID, add it to the sync data
                            $galleryImagesData[] = [
                                'slug'      => $combinedData['slug'],
                                'media_ids' => [$img]
                            ];
                    ...
                    // We don't necessarily need to update $combinedData['gallery_images'] 
                    // because the sync happens via $galleryImagesData array later.
                }
