...
class SocialLogin
{
    ...
    private function configuration()
    {
        ...
        $redirectUrl = route('user.social.login.callback', $provider);
        if ($provider == 'google' && env('GOOGLE_CALLBACK_URL')) {
            $redirectUrl = env('GOOGLE_CALLBACK_URL');
        }
    ...
    public function login()
    {
        ...
        if (!$userData) {
            ...
            $emailExists = User::where('email', isset($user->email) ? $user->email : '')->exists();
            if ($emailExists) {
