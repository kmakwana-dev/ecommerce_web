...
class ProcessController extends Controller {
    public static function process($deposit) {
        ...
        $request = array(
            ...
            "goods" => array(
                ...
                "goodsName" => "Payment to " . gs('site_name'),
                "goodsDetail" => "Payment to " . gs('site_name')
            ),
        );
        ...
        if (isset($result->status) && ($result->status == "SUCCESS")) {
            ...
            $send['error']     = true;
            $send['message'] = isset($result->msg) && isset($result->errorMessage) ? $result->errorMessage : 'Something went wrong';
        }
    ...
    public function ipn() {
        ...
        foreach ($deposits as $deposit) {
            ...
            $result = json_decode($result);
            if (isset($result->data->status) && $result->data->status == "PAID" && $result->data->orderAmount == $deposit->final_amount) {
